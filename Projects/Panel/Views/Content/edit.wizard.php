...
<div class="control-group">
        ...
        <div class="controls">
          <select name="category" id="" class="span11">
            <option value="">Category Select...</option>
              @foreach (content_category() as $key)
                  <option value="{{$key->id}}" {[selected(content_edit($id)->category_id, $key->id)]}>{{$key->name}}</option>
              @endforeach
          </select>
        </div>
      </div>
          ...
          <a href="/panel/content/delete/{{$id}}"><button class="btn btn-danger">DELETE</button></a>
        </div>
      </div>